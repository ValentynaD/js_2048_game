...
'use strict';
const game = new (0, _gameClassJs.Game)();
function updateUI() {
...
},{"../modules/Game.class.js":"eIbh2"}],"eIbh2":[function(require,module,exports,__globalThis) {
/* eslint-disable function-paren-newline */ /* eslint-disable comma-dangle */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
class Game {
    ...
    slideAndMerge(row) {
        ...
        const mergedRow = [];
        let skip = false;
        for(let i = 0; i < filteredRow.length; i++)if (!skip && i < filteredRow.length - 1 && filteredRow[i] === filteredRow[i + 1]) {
    ...
    moveDown() {
        const previousState = JSON.stringify(this.board);
        this.transpose();
        this.board = this.board.map((row)=>this.slideAndMerge(row.slice().reverse()).reverse());
        this.transpose();
        if (JSON.stringify(this.board) !== previousState) {
            this.spawnTile();
            this.checkGameOver();
    ...
    transpose() {
        this.board = this.board[0].map((_, colIndex)=>// eslint-disable-next-line prettier/prettier
            this.board.map((row)=>row[colIndex]));
    }
    checkGameOver() {
        if (!this.canMove()) this.status = 'lost';
        else if (this.board.flat().includes(2048)) this.status = 'won';
    }
    start() {
